...
namespace Kenet.Collections.Reactive
{
    ...
    public interface IListMutationTarget<TItem>
    {
        event EventHandler<Action<IMutationTarget<TItem>>> CollectRedirectionTargets;

        IList<TItem> Items { get; }

        /// <summary>
        /// Indicates whether <see cref="IMutationTarget{TItem}.ReplaceItem(int, Func{TItem}))"/> is functional and ready to be called.
        /// If it is <see langword="false"/> then it is intended that <see cref="IMutationTarget{TItem}.ReplaceItem(int, Func{TItem})"/>
        /// is not called.
        /// </summary>
        ...
        void Mutate(Action<IMutationTarget<TItem>> mutate, bool disableRedirection);
    }
}
